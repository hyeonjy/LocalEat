...
const TopTenRestaurantList = () => {
  ...
  // 데이터가 바뀌거나 첫 렌더 후에 상태 한 번 더 동기화
  useEffect(() => {
              ...
              <div className="h-[475px] w-full">
                <div className="flex gap-[20px]">
                  {Array.from({ length: SKELETON_DESKTOP_COUNT }).map(
                    (_, i) => (
                      <SkeletonDesktopCard key={`sk-d-${i}`} />
                    ),
                  )}
                </div>
              </div>
            ...
            {!isPending && (
              <>
                ...
                <button
                  ...
                  type="button"
                  aria-label="이전"
                  hidden={!ready || atStart}
                  className="absolute left-[-20px] top-1/2 z-10 hidden -translate-y-1/2 items-center justify-center rounded-[1000px] border border-[#E2E2E4] bg-[#FFF] p-[8px] transition-opacity xl:flex [&.swiper-button-disabled]:pointer-events-none [&.swiper-button-disabled]:opacity-0"
                >
                  <Image
                ...
                <button
                  ...
                  aria-label="다음"
                  hidden={!ready || atEnd}
                  className="absolute right-[-20px] top-1/2 z-10 hidden -translate-y-1/2 items-center justify-center rounded-[1000px] border border-[#E2E2E4] bg-[#FFF] p-[8px] transition-opacity xl:flex [&.swiper-button-disabled]:pointer-events-none [&.swiper-button-disabled]:opacity-0"
                >
                  <Image
                ...
                <Swiper
                  ...
                  onInit={(sw) => {
                    ...
                    setAtStart(sw.isBeginning);
                    setAtEnd(sw.isEnd);
                    // 다음 프레임에 ready=true → disabled 반영이 끝난 뒤 버튼 DOM 노출
